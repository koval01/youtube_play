...
'<a href="mailto:$1">$1</a>')},numberWithCommas=a=>a.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,","),getRandomInt=(a,b)=>{a=Math.ceil(a);b=Math.floor(b);return Math.floor(Math.random()*(b-a)+a)},repeatStringNumTimes=(a,b)=>{for(var c="";0<b;)c+=a,b--;return c},requestCall=(a,b,c,d=!1,f=null)=>{let e=new XMLHttpRequest,g={};e.open(c,b,!0);"POST"===c.toUpperCase()&&(e.setRequestHeader("Content-Type","application/json;charset=UTF-8"),g=JSON.stringify(f));e.onload=function(){200<=e.status&&
400>e.status?d?a(JSON.parse(e.responseText)):a(e.responseText):console.log(`Request status code is ${e.status}`)};e.onerror=function(h){console.log(`Error make request! Details: ${h}`);a(null)};e.onreadystatechange=()=>{400<=e.status&&(d?a({success:!1}):a(null))};e.send(g)},getVideo=(a,b)=>{requestCall(function(c){c?c.success?a(c.body):(notify("Server-side function error"),errorLoadVideo()):(notify("Application server error"),errorLoadVideo())},`${backend_host}/getVideo`,"POST",!0,{video_id:b})},
updateVideoObserver=()=>{let a=document.querySelector("video");a.onpause=b=>{playControl(b)};a.onplay=b=>{video_loading=!1;playControl(b)};a.onwaiting=b=>{video_loading=!0;playControl({type:"pause"})};a.onplaying=b=>{video_loading=!1;playControl({type:"play"})}},updateVideo=()=>{let a=document.getElementById("video-container");notify("Please wait, receiving data...");getVideo(function(b){var c="";if(b.release_timestamp){var d=new Date(1E3*b.release_timestamp);c=`
                <span class="dot"></span>
                <p>${d.toLocaleString("default",{month:"short"}).slice(0,3)} ${d.toLocaleString("default",{day:"numeric"})}, ${d.getFullYear()}</p>
            `}d=b.formats.audio.url;var f=b.formats.video.q480p.url,e=b.title,g=numberWithCommas(b.view_count),h=b.like_count?numberWithCommas(b.like_count):"",l=linkify(b.description.replaceAll("\n","<br/>"));b=b.tags;let k="";if(b)for(let m of b)k+=`#${m}\x20`;a.innerHTML=`
        ...
        `;updateVideoObserver()},video_id_link)},notify=a=>{let b=document.querySelector(".error_box_cst"),c=document.querySelector(".error_text_cst"),d=function(){b.style.marginBottom="-150px";notify_hidden=!0},f=function(){notify_hidden=!1;c.innerHTML=a;b.style.marginBottom="0"};notify_hidden?f():(d(),setTimeout(f,200));clearTimeout(timer_notify);timer_notify=setTimeout(d,2500)},isAttentionSplash=()=>{Cookies.get("attention_close")||(document.querySelector(".is-attention-splash").style.display=
"")},closeAttentionSplash=()=>{let a=document.querySelector(".is-attention-splash");a.style.marginTop="-100vh";setTimeout(function(){a.style.display="none"},600);Cookies.set("attention_close","1")},generatePseudoLine=(a,b=[5,20])=>{let c="";for(let d=0;d<a;d++)c+=repeatStringNumTimes("-",getRandomInt(b[0],b[1]))+" ";return c},errorLoadVideo=()=>{document.getElementById("video-container").innerHTML=`
    ...
    `},syncTime=()=>{let a=document.querySelector("video"),b=document.querySelector("audio");video_loading||(b.currentTime=a.currentTime)},playControl=a=>{let b=document.querySelector("audio");"play"===a.type?(syncTime(),b.play().then(c=>{})):"pause"===a.type&&b.pause()},loadingFinish=()=>{let a=document.querySelector(".page-loading");setTimeout(function(){a.classList.remove("active")},500);setTimeout(function(){a.remove()},600)};
