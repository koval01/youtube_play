...
'<a href="mailto:$1">$1</a>')},numberWithCommas=a=>a.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,","),getRandomInt=(a,b)=>{a=Math.ceil(a);b=Math.floor(b);return Math.floor(Math.random()*(b-a)+a)},repeatStringNumTimes=(a,b)=>{for(var c="";0<b;)c+=a,b--;return c},request_call=(a,b,c,e=!1,f=null)=>{let d=new XMLHttpRequest,g={};d.open(c,b,!0);"POST"===c.toUpperCase()&&(d.setRequestHeader("Content-Type","application/json;charset=UTF-8"),g=JSON.stringify(f));d.onload=function(){200<=d.status&&
400>d.status?e?a(JSON.parse(d.responseText)):a(d.responseText):console.log(`Request status code is ${d.status}`)};d.onerror=function(h){console.log(`Error make request! Details: ${h}`);a(null)};d.onreadystatechange=()=>{400<=d.status&&(e?a({success:!1}):a(null))};d.send(g)},get_video=(a,b)=>{request_call(function(c){c?c.success?a(c.body):(notify("Server-side function error"),error_load_video()):(notify("Application server error"),error_load_video())},`${backend_host}/get_video`,"POST",!0,{video_id:b})},
update_video_observer=()=>{let a=document.querySelector("video");a.onpause=b=>{play_control(b)};a.onplay=b=>{video_loading=!1;play_control(b)};a.onwaiting=b=>{video_loading=!0;play_control({type:"pause"})};a.onplaying=b=>{video_loading=!1;play_control({type:"play"})}},update_video=()=>{let a=document.getElementById("video-container");notify("Please wait, receiving data...");get_video(function(b){var c=new Date(1E3*b.epoch),e=b.formats.audio.url,f=b.formats.video.q480p.url,d=b.title,g=numberWithCommas(b.view_count),
h=c.toLocaleString("default",{month:"short"}).slice(0,-1),l=c.getDay();c=c.getFullYear();var m=numberWithCommas(b.like_count),n=linkify(b.description.replaceAll("\n","<br/>"));b=b.tags;let k="";if(b)for(let p of b)k+=`#${p}\x20`;a.innerHTML=`
